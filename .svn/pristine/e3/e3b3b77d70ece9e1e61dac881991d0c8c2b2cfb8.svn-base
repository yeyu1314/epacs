  ...
  <div class="warping">
          ...
          <el-table-column fixed="right" label="操作" align="center">
            <template slot-scope="scope">
              ...
              <el-button
                ...
                v-show="scope.row.jobCode==210 || scope.row.jobCode==220|| scope.row.jobCode==221 "
                type="warning"
                size="small"
              >编辑检测报告</el-button>
            </template>
    ...
    <div v-show="isShowOpear">
      ...
      <transition name="slide-fade">
        <div v-show="showTemplate" class="leftTool" @mouseleave="mouseLe">
          <p style="margin:15px 0 10px 10px;">*检测结果模板</p>
          <el-select
            v-model="engineType"
            clearable
            placeholder="请选择部位"
          ...
          >
            <el-option
              v-for="(item,index) in gasolineArr"
              :key="item.id"
              :label="item.name"
              :value="item.name"
            >
              <span
                style="float: left; color: #8492a6; font-size: 13px;margin-right: 0px;"
              >{{ index +1 }}：</span>
      ...
      <div class="centent" id="firstContent">
        <div style="background:aliceblue;" @mouseover="showTemplate = !showTemplate">模板</div>
        <!-- 中间 -->
        <div
          id="centershow"
...
export default {
  ...
  created() {
    ...
    if (this.operId == 1 || this.operId == 2) {
  ...
  methods: {
    ...
    //编辑报告
    edit(row) {
      this.exChange();//监听说明 意见
      this.isShowOpear = true;//显示编辑页面
      this.isShowSub = false;// 提交报告
    ...
    engineChange(val) {
      ...
      net
        .request(
          "admin/select/getPreliminaryTemplet",
          "post",
          {
            optionId: val
          },
    ...
    unqualified() {
      ...
      //   if(this.unqualifiedArr[i].optionName.substr(-7)=="(已上传图片)"){
      //     this.position.push(this.unqualifiedArr[i])
      //   }
      // }
    ...
    exChange() {
      $("#exCtrl").on("input propertychange", e => {
        this.exLen = 260 - e.target.value.length;
      });
      $("#chCtrl").on("input propertychange", e => {
        this.chLen = 120 - e.target.value.length;
      });
    },
    //查看记录
    lockOver(row) {
    ...
    mouseLe(e) {
      var sign = $(e.toElement).attr("class");
      // console.log(sign,sign == "el-scrollbar__view el-select-dropdown__list")
      if (
        sign == "el-scrollbar__view el-select-dropdown__list" ||
        sign == "el-select-dropdown el-popper" ||
        sign == "el-select-dropdown__item selected hover" ||
        sign == "el-select-dropdown__item" ||
        sign == "popper__arrow" ||
        sign == "el-select-dropdown__item selected"
      ) {
